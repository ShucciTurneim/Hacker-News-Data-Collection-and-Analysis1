...
def load_stories_data_to_file(data,file_name,stories_number_to_analyse):
    ...
    for ID in range(stories_number_to_analyse):
        ...
        if ID_data != 'error':
            row_to_write = []
            row_to_write.append(ID_data['id']) if 'id' in ID_data  else ''
...
def load_comments_data_to_file(story,comments_id,comments_number_to_analyse):
    for ID in range(comments_number_to_analyse):
        ID_data = try_data(story)
        

def comments_data_to_csv(stories_id,comments_id,headlines_comments):
    for story in stories_id:
        file_name = init_csv_data(headlines_comments,f'{story}_comments')
        load_comments_data_to_file(story,file_name,5)


def  collection_and_analysis():
    headlines_stories = [ 'ID','title', 'URL', 'score', 'author', 'time', 'number of comments']
    headlines_comments = ['author', 'text', 'time','parent_story']
    ...
    comments_data_to_csv(stories_id, comments_id, headlines_comments)
